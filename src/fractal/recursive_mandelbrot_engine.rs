...
impl RecursiveMandelbrotEngine {
    ...
    fn calculate_recursive(&self, region: &Region, window: &Window, pixel_offset: u32, pixels: &mut [u8]) {
        let unique_escape = self.unique_escape_for(region, window);

        if let Some(escape) = unique_escape {
            self.fill_window(region, window, escape, pixel_offset, pixels);
            return;
        }
        ...
        if let Some(part2) = optional_part2 {
            self.calculate_recursive(region, &part1, pixel_offset, pixels);
            self.calculate_recursive(region, &part2, pixel_offset, pixels);
            return;
        }

        self.calculate_window(region, window, pixel_offset, pixels);
    }


    fn unique_escape_for(&self, region: &Region, window: &Window) -> Option<u8> {
    ...
    fn fill_window(&self, region: &Region, window: &Window, escape: u8, pixel_offset: u32, pixels: &mut [u8]) {
    ...
    fn calculate_window(&self, region: &Region, window: &Window, pixel_offset: u32, pixels: &mut [u8]) {
        let window_area_pixels = WindowAreaIterator::new(window);
...
impl MandelbrotEngine for RecursiveMandelbrotEngine {
    ...
    fn calculate_serially(&self, region: &Region, pixels: &mut Vec<u8>) {
        ...
        self.calculate_recursive(region, &window, 0, pixels);
    }
    ...
    fn calculate_in_parallel(&self, region: &Region, pixels: &mut Vec<u8>) {
        ...
        pixel_bands.into_par_iter()
            .for_each(|(band_nr, pixel_band)| {
                ...
                self.calculate_recursive(region, &window_band, pixel_offset, pixel_band);
            });
    }
}
